...
build PRESENTATION FORMAT='html': install
    ...
    npx marp "{{PRESENTATION}}" --output "$output"
...
build-all FORMAT='html': install
    ...
    for file in presentations/*.md; do
        ...
        echo "  - Building $base"
        output="dist/${base%.md}.{{FORMAT}}"
        npx marp "$file" --output "$output"
    done

[group('dev')]
dev PRESENTATION: install
    npx marp {{PRESENTATION}} --server --watch --html

[group('dev')]
dev-all: install
    npx marp --server --watch --html presentations
...
precommit: install
    ...
    for file in presentations/*.md; do
        if [ -f "$file" ]; then
            base=$(basename "$file")
            echo "  - Building $base"
            npx marp "$file" --output "dist/${base%.md}.html"
        fi
    done
